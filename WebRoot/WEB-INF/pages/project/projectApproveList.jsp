...
<body>
   <div class="right_main">
    <div class="add_top"><span style="float: left;">您当前的位置：中小企业服务信息公开 > 发布审批信息</span>
    	<s:if test="session.loginUser.site.type==1">
	  	<span style="float:right"><a href="project/projectApproveConfig_initAdd.action">配置</a>&nbsp;&nbsp;&nbsp;&nbsp;</span></s:if>
    </div>
	...
	结束时间:<input type="text" id="endTime" name="endTime"  onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd '})" class=" Wdate " style="width: 100px" readonly="readonly"/>	
		...
		选择栏目：
		 <select name="scolumnName" id="scolumnName"> 
      <s:iterator value="scolumnList" var="scolumnList" > 
       	    <option value="${scolumnList.name}">${scolumnList.name}</option> 	    
	   </s:iterator> 
	   </select>
    <div class="listop_search">审批信息
  
    <!--    <div class="search_text"> --> 
     
    <s:if test="session.loginUser.role.id==8">
	...
	<s:else>
		...
		<a href="project/projectApprove_initAdd.action?type=1">
  ...
  <!--   <form id="fom" method="post" action="project/projectApprove_batchDelete.action"> -->
    ...
    <tr>
      ...
      <th align="center"><input id="selectAll" type="checkbox"  onclick="checkAll()">全选</th>  
      <th>标题</th>
      <th>所属栏目</th>
      <th>发布平台</th>
      <th>发布时间</th>
     ...
     <tr>
        <td align="center"><input type="checkbox" name="proIds"  value="${pro.id}"/></td>
