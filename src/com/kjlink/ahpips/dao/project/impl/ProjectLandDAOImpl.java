...
@Component("projectLandDAO")
public class ProjectLandDAOImpl extends BaseDAOImpl<ProjectLand> implements ProjectLandDAO {

	public ProjectLandDAOImpl() {
		super(ProjectLand.class);
	}
	
	public List<ProjectLand> findProjectLandByProject(Project project){
		String hql = "from ProjectLand pl where pl.project.id="+project.getId();
		return this.findByHql(hql);
	}
	
	public List<ProjectLand> findProjectLandForPulbic(int count,Site site){
		String hql = "from ProjectLand pl where 1=1  ";
...
//		}
	...
	public PageInfo findPageAll(int pageNo, int pageSize){
		String hql = "from ProjectLand pl order by pl.id desc";
...
//		}
	...
	public PageInfo quickSearch(String str, int pageNo, int pageSize) {
		...
		String hql = "from ProjectLand pl where pl.project.name like ? order by pl.id desc";
	...
	public PageInfo findProjectLandBySiteId(String str, String sitecode,
		...
		String hql = "from ProjectLand pl where ( pl.name like ? or pl.code like ? ";
	...
	public PageInfo findByToDo(String str,User user, int pageNo, int pageSize){
		String tmp = str == null ? "" : str;
		String hql = "from ProjectLand pl where pl.name like ? and pl.site.code=? and (pl.approveUser.id="+user.getId()+" or pl.approveUser=null) and pl.state=3 order by pl.id desc";
		return this.findPageInfoByHql(pageNo, pageSize, hql, new String[] {"%"+tmp+"%",user.getSite().getCode()});
	}
	public int findCount(User user){
		String hql = "select count(*) from ProjectLand pl where pl.site.id="+user.getSite().getId()+" and (pl.approveUser.id="+user.getId()+" or pl.approveUser=null) and pl.state=3 order by pl.id desc";
	...
	public int findDataCount(Site site){
		String hql = "select count(*) from ProjectLand pl where ( pl.site.id="+site.getId()+" or (pl.site.parent.id="+site.getId()+" and pl.site.type=0)) and pl.state=1 ";
	...
	public int findDataCount(Site site,Date startTime,Date endTime){
		...
		String hql = "select count(*) from ProjectLand pl where pl.site.id="+site.getId()+" and pl.state=1 ";
	...
	public int findByUser(User user){
		String hql = "select count(*) from ProjectLand pl where  pl.createUser.id="+user.getId()+"  and pl.state=1 ";
